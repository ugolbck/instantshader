...
- 174544a: Effects: shaders that redraw an existing picture, over a gradient or over your
  own image.
...
- 22e73fe: Four new shaders, each with a matching React component:
...
- 0c3b5b4: New "Bloom" shader: an analytic fan of soft petals radiating from the frame's
  bottom edge, with the palette laid out radially (hot core to dark background).
  Two blendable motion modes via params: `sway` (petals breathe and lean) and
  `colorflow` (colors travel outward through a static pattern).

### Patch Changes

- Updated dependencies [0c3b5b4]
  - instantshader@0.4.0
