...
Five new looks: `silk` (satin folds with an anisotropic sheen; matte and grainy at `sheen: 0`), `wisp` (molten threads on dark with hot beads and sparks), `nacre` (glassy liquid folds whose colour shifts through the palette on every slope), `burst` (thin grainy rays out of a dark core) and `glint` (a shoal of spinning shards streaming along a current). React: `<Silk>`, `<Wisp>`, `<Nacre>`, `<Burst>`, `<Glint>`.
